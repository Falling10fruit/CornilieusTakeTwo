...
@compute @workgroup_size(32) fn setup_collision_nodes(
    ...
    let former_type_id = (collision_vector.z >> 12) & 0x3FFu;
...
fn support_function(direction : vec2f) -> SupportNode {
    let first_vertex_data = private_entity_nodes[0];
    let first_node_vector = vec4f8_to_vec4f32(vec4u(
        first_vertex_data & 0xFFu, (first_vertex_data >> 8) & 0xFFu,
        (first_vertex_data >> 16) & 0xFFu, (first_vertex_data >> 24) & 0xFFu,
    ));
    ...
    let vertex_count_max = node_meta & 0xFu;
    for (var i : u32 = 1; i < vertex_count_max; i++) {
        let vertex_data = private_entity_nodes[0];
        let vertex_vector = vec4f8_to_vec4f32(vec4u(
            vertex_data & 0xFFu, (vertex_data >> 8) & 0xFFu,
            (vertex_data >> 16) & 0xFFu, (vertex_data >> 24) & 0xFFu,
        ));

        let former_rotated = rotate_vertex(vertex_vector.xy, cosin_lut[(rotations & 0xFFFu) << 1]);
        let latter_rotated = rotate_vertex(vertex_vector.zw, cosin_lut[(rotations >> 12) << 1]);

        let former_dot_product = dot(former_rotated, direction);
        ...
        let latter_dot_product = dot(latter_rotated, direction);
...
fn rotate_vertex(vertex : vec2f, cosin : vec2f) -> vec2f {
    return vec2f(
        vertex.x * cosin.x - vertex.y * cosin.y,
        vertex.x * cosin.y + vertex.y * cosin.x
    );
}
