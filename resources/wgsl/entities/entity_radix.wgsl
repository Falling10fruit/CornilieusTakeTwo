...
@group(0) @binding(1) var<storage> entity_buffer_0 : array<vec4u>;
@group(0) @binding(2) var<storage> entity_buffer_1 : array<vec4u>;
...
@compute @workgroup_size(256) fn sort_entities( //
    ...
    let chunk_byte = (entity_buffer_0[global_invocation_id.x].x >> (7 + BYTE_SHIFT)) & 0xFFu;
    let offset =    
}